...
- The `guard-destructive.sh` PreToolUse hook mishandled pushes that target a repo other than the project dir, in both directions. Fail-open: `git -C <path> push` never matched the guard's case arm (it required the literal substring "git push"), so a force-push or push-to-main routed through `-C` sailed past every check. False positive: the implicit-branch check always consulted `$CLAUDE_PROJECT_DIR`, so `cd <elsewhere> && git push` from another repo's feature branch was refused whenever *this* project sat on main. The push arm now also matches the `-C` form, and the branch check resolves the repo the push actually runs in — an explicit `-C` on the invocation, else the last `cd` before the push, else the project dir; an unresolvable path keeps the project-dir fallback, which can only over-block. Nine new bats regressions cover both defects.
